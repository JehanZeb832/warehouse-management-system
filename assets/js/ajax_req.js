...
// user update ajax  
$("#edituserfForm").submit(function (event) {
  /* Act on the event */
  event.preventDefault();
  var data = $('#edituserfForm').serialize();
  $.ajax({
    type: 'POST',
    url: "app/action/edit_user.php",
    data: data,
    success: function(data){
      alert(data);
    }
...
// user update ajax  
$("#edituserRoleForm").submit(function (event) {
  /* Act on the event */
  event.preventDefault();
  var data = $('#edituserRoleForm').serialize();
  $.ajax({
    type: 'POST',
    url: "app/action/edit_userRole.php",
    data: data,
    success: function(data){
      alert(data);
    }
...
// staff delete content js 
$(document).on('click', '#user_delete_btn', function(event) {
  event.preventDefault();
  /* Act on the event */
    $delete_id = $(this).data('id');
    var confirm_delete = confirm("Are You sure want to delete this item?")
    if (confirm_delete) {
    $.post('app/action/delete_user.php', {delete_id : $delete_id , delete_data : 'delete_data'}, function(data) {
      if ($.trim(data) == 'true') {
        alert('data deleted successfull');
         location.reload();  
      }else{
